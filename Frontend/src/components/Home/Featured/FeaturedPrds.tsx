...
const FeaturedPrds = ({ products }) => {
    ...
    useEffect(() => {
        ...
        const updateProductLimit = () => {
            ...
            // If screen is 'md' (768px) or larger, but not 'xl' (1280px) or larger
            if (width >= 768 && width < 1280) {
                setProductLimit(3); // md: 3 items
            } else {
                setProductLimit(4); // xs, sm, xl, and up: 4 items
            }
        };
    ...
    return (
        <div className='m-4 sm:m-6 lg:m-10 p-4 sm:p-5 flex flex-col gap-6 sm:gap-8 lg:gap-10'>
            <div className='items-center justify-center flex flex-col text-center px-2'>
                <h1 className='font-bold text-xl sm:text-2xl lg:text-3xl dark:text-white'>Featured Products</h1>
                <div className="h-[2px] sm:h-[3px] w-12 sm:w-16 mx-auto mt-1 bg-[#d5754d]"></div>
                <p className='mt-2 sm:mt-3 font-semibold dark:text-white/80 text-sm sm:text-base'>Our most popular and highly-rated fireworks</p>
            </div>

            {/* cards */}
            <div>
                {/* 2. Used `productLimit` in the slice */}
                <ul className="grid grid-cols-1 mx-10 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-15">
                    {localProducts?.slice(0, productLimit).map((prd: any) => (
                        <li key={prd.slug} onClick={() => item(prd.slug)} className="group bg-white dark:text-white dark:bg-gray-800 hover:shadow-2xl dark:hover:shadow-[#d5754d]/30 rounded-xl border-1 border-[#d5754d] hover:cursor-pointer p-3 sm:p-4 flex flex-col items-center hover:translate-x hover:-translate-y-2 sm:hover:-translate-y-4 transition-all duration-400 gap-2">

                            <div className='relative flex flex-col gap-1 w-full'>
                                {/* wish */}
                                <div className='ml-auto'>
                                    {localWish?.some((wish: any) => wish.product.slug === prd.slug) ?
                                        (<HiMiniHeart className='text-red-500 text-base sm:text-lg' onClick={() => addWish(prd)} />) :
                                        <HiOutlineHeart className='text-base sm:text-lg' onClick={(e) => { e.stopPropagation(); addWish(prd) }} />
                                    }
                                </div>

                                {/* prd img */}
                                <div className="flex mt-2">
                                    <img src={prd.image ? prd.image : '/assets/Product/Product.webp'} className="w-full h-40 sm:h-50 rounded-lg transition duration-300 group-hover:blur-sm" />
