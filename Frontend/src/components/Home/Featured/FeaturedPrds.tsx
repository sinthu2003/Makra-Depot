  ...
  return (
      <div className='m-10 p-5 flex flex-col gap-10'>
        <div className='items-center justify-center flex flex-col'>
          <h1 className='font-bold text-3xl dark:text-white'>Featured Products</h1>
          <div className="h-[3px] w-1/16 mx-auto mt-1 bg-[#d5754d]"></div>
          <p className='mt-3 font-semibold dark:text-white/80'>Our most popular and highly-rated fireworks</p>
        </div>
        {/* cards */}
        <div>
          <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-15">
            {localProducts?.map((prd : any) => (
              <li key={prd.slug} onClick={()=>item(prd.slug)} className="group bg-white dark:text-white dark:bg-gray-800 hover:shadow-2xl  dark:hover:shadow-[#d5754d]/30 rounded-xl border-1 border-[#d5754d] hover:cursor-pointer p-4 flex flex-col items-center hover:translate-x hover:-translate-y-4 transition-all duration-400 gap-2">
        ...
        <div className='flex justify-center'>
          <Link to="/products"><button className='px-4 py-3 bg-black text-white rounded-xl font-semibold text-xs hover:shadow-2xl  dark:hover:shadow-[#d5754d]/50 transition-all duration-300 hover:cursor-pointer w-0-0.5/6  whitespace-nowrap hover:scale-110 dark:bg-white dark:text-gray-900'>View All Featured Products</button></Link>
        </div>
